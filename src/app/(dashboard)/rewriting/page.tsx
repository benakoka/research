...
export default function RewritingPage() {
  ...
  const [runs, setRuns] = useState<RewritingRun[]>([]);
  const [starting, setStarting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/rewriting/runs")
      .then((r) => r.json())
      .then(setRuns);
  }, []);
  ...
  async function startRun() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/rewriting/runs", {
        method: "POST",
      ...
      router.push(`/rewriting/runs/${body.id}`);
    } catch (err) {
          ...
          <ul className="divide-y divide-slate-200 rounded-xl border border-slate-200 bg-white">
            {runs.map((r) => (
              <li key={r.id} className="flex items-center justify-between px-4 py-3 text-sm">
                <div>
                  <a href={`/rewriting/runs/${r.id}`} className="font-medium text-slate-800 underline">
                    Run {r.id.slice(0, 8)}
                  </a>
                  <div className="text-slate-500">
                    {new Date(r.createdAt).toLocaleString()} · GPT {r.gptModelSnapshot || "(no snapshot)"} ·
                    {" "}Gemini {r.geminiModelSnapshot || "(no snapshot)"} · {r.chainIds.length} chains
                  </div>
                </div>
                <span
                  className={
